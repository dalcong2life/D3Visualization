...
???
차트를 표현하는 방법은 다양하게 있고 어떻게 표현 될것인지는 데이터 구조에 영향을 받는다.<br>

시계열 데이터와 장르별 매출 데이터, 앙케이트 결과등은 배열로<br>
트리나 소셜 네트워크의 인간관계 웹페이지 액세스 흐름등은 그래프 구조로 나타낼 수 있다.
...
???
배열 데이터를 이용해서 표현 할 수 있는 차트의 종류와 성질을 살펴본다.
...
??? 
웹 사이트 가입 단계 또는 쇼핑몰 방문, 장바구니에 넣기, 배송정보 입력, 구매, 결제에 이어지는 각 단계에서 그쳤는지 분석 
...
???
방사형 차트는 평행으로 그린 병렬형 차트의 선을 중심으로부터 방사형으로 그린 차트를 말한다.<br>
한정된 공간에서의 표현에 어울리지만 극좌표 표현은 사람이 인지하기 어렵기 때문에 평행형 차트를 사용
